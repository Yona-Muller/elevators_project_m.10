...
class Elevator:
    ...
    def pop_task(self):
        self.__current_floor = self.__tasks_queue[-1][0][1]
    ...
    def tasks_time(self, floor):
        ...
        return abs(self.__current_floor - floor) * 0.5 + self.__time_left + 2 if not self.__tasks_queue else abs(self.__tasks_queue[-1][0][1] - floor) * 0.5 + self.__tasks_queue[-1][1] + self.__time_left

    def move(self):
        if self.__ele_status["standing"] and self.__tasks_queue:
            self.__ele_status["moving"], self.__time_task = self.pop_task()
            self.__time_task -= 2
