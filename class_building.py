    ...
    def move(self, screen, click_position):
        for floor in self.__floors:
            if floor.get_image_rect().centerx - 15 <= click_position[0] <= floor.get_image_rect(
                ...
                screen.fill(white)
                dy = floor.get_image_rect().centery - self.__elevators[0].get_image_rect().centery
                # print(elevator.get_num_elevator)
                # Move the image gradually towards the target position
                if dy > speed:
                    # dy *= speed
                    self.__elevators[0].get_image_rect().centery += speed
                if dy < speed:  
                    self.__elevators[0].get_image_rect().centery -= speed
                for elevator in self.__elevators:
