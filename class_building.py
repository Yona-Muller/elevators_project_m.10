...
class Building:
    ...
    def optimal_elevator(self, target_floor: Floor):
        ...
        if not elevator.no_tasks():
            target_floor.set_timer(abs(target_floor.get_floor_num(
            ) - elevator.get_lest_task()) * 0.5 + self.__elevators[min[1]].get_time_tasks())
        else:
            target_floor.set_timer(
                abs(target_floor.get_floor_num() - elevator.get_current_floor()) * 0.5)
    ...
    def drawing_floors_and_elevators(self, screen):
        ...
        for floor in self.__floors:
            ...
            screen.blit(text, (data["width_floor"] *
                        0.73, floor.get_image_rect().centery - 4))
