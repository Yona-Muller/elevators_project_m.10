...
def call_topleft_window(num_ele, num_floors, screen_size):
    y = (screen_size[0][1] - (data["space_left"] * 2 + data["width_floor"] + num_ele * data["width_ele"])) // 2
    x = (screen_size[0][0] - (data["space_down"] * 2 + num_floors * data["height_floor"])) // 2
    print(screen_size)
    print(data["space_left"] * 2 + data["width_floor"] + num_ele * data["width_ele"])
    print(data["space_down"] * 2 + num_floors * data["height_floor"])
    print(x, y)
    return x, y

def set_screen_position(num_ele, num_floor, screen_size):
    x, y = call_topleft_window(num_ele, num_floor, screen_size)
